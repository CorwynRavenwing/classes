...
def hackerlandRadioTransmitters(x, k):
    ...
    while working_index < len(x):
        ...
        working_index = top_index
        time_profile('range split; popping')
        while working_index < len(x) and x[working_index] == top_of_range:
            working_index += 1
            print("#POP")
        time_profile('done popping')
        # print(f"#{in_range=}")
        print(f"#next range={x[working_index:working_index+5]}")
