...
def highest_value_less_than(V, arr, chunks):
    # grab another chunk if arr is too low
    if chunks:
        while len(chunks) and len(arr) and max(arr) <= V:
            print("#GRAB NEXT CHUNK")
            next_chunk = chunks.pop(0)
            arr.extend(next_chunk)
            # change arr and chunks in a way that
            # passes back out of this function
    if not len(chunks):
...
def hackerlandRadioTransmitters(x, k):
    ...
    while len(x) or len(chunks):
        ...
        t = highest_value_less_than(h1+k, x, chunks)
        time_profile('got T')
        print(f"#{t=}")
        transmitters += 1

        time_profile('calling HVLT for TOR')
        top_of_range = highest_value_less_than(t+k, x, chunks)
