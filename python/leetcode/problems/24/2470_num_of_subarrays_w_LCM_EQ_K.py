class Solution:
    def subarrayLCM(self, nums: List[int], k: int) -> int:
        ...
        # k_factor = factor(k)
        # print(f'{k_factor=}')
        # print(f'{k=} {number_from_factor(k_factor)=}')
        
        # print(f'{factor(1)=}')
        # print(f'{LCM_F(factor(15), factor(6))=}')

        answer = 0
        lcm_of_range = 1
        for i, Ni in enumerate(nums):
            lcm_of_range = Ni
            # print(f'[{i}] {lcm_of_range}')
            if lcm_of_range == k:
                # print(f'  YES')
                answer += 1
            elif lcm_of_range > k:
            ...
            for j in range(i + 1, len(nums)):
                Nj = nums[j]
                lcm_of_range = LCM(lcm_of_range, Nj)
                # print(f'  [{i}:{j}] {Nj}:{lcm_of_range}')
                if lcm_of_range == k:
                    # print(f'    YES')
                    answer += 1
                elif lcm_of_range > k:
                    # print(f'    >K: break j, next i')
                    break

        return answer
# NOTE: Runtime 290 ms Beats 54.17%
# NOTE: Memory 17.75 MB Beats 5.21%
